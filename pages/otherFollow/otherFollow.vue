<template>
	<view class="page">
		<view style="margin:0 auto;font-weight: 700;font-size: 48rpx;"> 他的关注</view>
		<u-swipe-action :show="item.show" :btn-width="btnWidth" :index="index" v-for="(item, index) in list" :key="item.id" @click="click(item.id)"
			...
			<view class="item u-border-bottom">
				<image mode="aspectFill" :src="item.avatar" @click="toOther(item.id)" />
	...
	export default {
		...
		methods: {
			...
			getFollows() {
				...
				let _self=this;
				this.$request('/follow/followsById/'+_self.id, '', 'GET').then(result => {
					console.log(result);
